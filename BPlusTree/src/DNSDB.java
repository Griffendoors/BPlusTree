...
public class DNSDB {
	...
	public void iterateAll(){
		StringNode yes = ipAddresses.root;					// Somewhere to start
		String toPrint = "No Values to print!";				// Variable to hold printing value
		while(true){										// This loop gets to the base of the base of the tree, then the far left key
			if(yes instanceof LeafString){					// if a leaf, at bottom of tree
				toPrint = yes.keys[0];						// print variable is far left key
				break;
			}
			else{
				yes = ((InternalString)yes).Children[0];	// not a leaf, increment to layer below
			}
		}


		while(true){											// do the actual printing
			System.out.println(yes.toString());					// print the keys info
			if(((LeafString)yes).nextNode==null) break;			// if at end of key chain
			yes = ((LeafString)yes).nextNode;					// move onto next
		}
